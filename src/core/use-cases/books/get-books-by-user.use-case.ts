import { databaseGetBooksByUser, databaseSearchBookByIsbn } from '../../../infrastructure/database/books.repository';
import { DatabaseBook, UserBook } from '../../../infrastructure/interfaces/supabase.responses';
import { Book } from '../../entities/book.entity';

/**
 * Gets the list of books in a user's collection, combining user-specific data with general book information from the database.
 * @returns A list of books in a user's collection, combining user-specific data with general book information from the database.
 */
export const getBooksByUser = async (user: string, offset?: number, limit?: number): Promise<Book[]> => {

    try {
        const userBooks: UserBook[] = await databaseGetBooksByUser(user, offset, limit);
        ...
        const books: Book[] = userBooks.map((userBook, idx) => {
        ...
        throw new Error(`Error fetching books by user: ${error}`);
    }
};
