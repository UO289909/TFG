...
export const AddBookScreen = () => {
	...
	const { params } = useRoute<RouteProp<RootStackParams, 'AddBook'>>();
	...
	useEffect(() => {
		if (search !== null) {
			handleRemainingFields(search.book, search.fromOpenLibrary);
		}
	}, [search])
	...
	const handleSearchPress = (text: string) => {
		...
		if (/^\d+$/.test(text)) {
			...
			setFieldsEnabled(['isbn']);
			navigation.navigate('BookSearch', { query: normalizeText(text.trim()) });
		}

	}

	const handleRemainingFields = (book: Book, fromOpenLibrary: boolean) => {
	...
	const handleSearchISBN = async (text: string) => {
		...
		try {
			...
			if (book !== null) {
				...
				handleRemainingFields(book, fromOpenLibrary);
